...
class Giay extends DB{
    ...
    {
        $sql="select * from product join dacdiem_sp on product.ma_sp = dacdiem_sp.ma_sp where product.ma_sp = $id order by size asc";
        return $this->select($sql);
    }
    public function add_product($tensp, $motasp, $giasp, $ma_th, $link_hinhanh, $ma_danhmuc, $phan_loai) {
        $sql = "insert into product (tensp, motasp, giasp, ma_th, link_hinhanh, ma_danhmuc, phan_loai) 
                values (?, ?, ?, ?, ?, ?, ?)";
        return $this->insert($sql, [$tensp, $motasp, $giasp, $ma_th, $link_hinhanh, $ma_danhmuc, $phan_loai]);
    }
    public function delete_product($id) {
        $sql = "delete from product where ma_sp = ?";
        return $this->delete($sql, [$id]);
    }
    public function update_product($id, $tensp, $motasp, $giasp, $ma_th, $link_hinhanh, $ma_danhmuc, $phan_loai) {
        if ($link_hinhanh != '') {
            $sql = "update product set tensp=?, motasp=?, giasp=?, ma_th=?, link_hinhanh=?, ma_danhmuc=?, phan_loai=? where ma_sp=?";
            return $this->update($sql, [$tensp, $motasp, $giasp, $ma_th, $link_hinhanh, $ma_danhmuc, $phan_loai, $id]);
        } else {
            $sql = "update product set tensp=?, motasp=?, giasp=?, ma_th=?, ma_danhmuc=?, phan_loai=? where ma_sp=?";
            return $this->update($sql, [$tensp, $motasp, $giasp, $ma_th, $ma_danhmuc, $phan_loai, $id]);
        }
    }

    
    public function loc_san_pham($ma_danhmuc, $phan_loai, $sap_xep, $brand, $size, $mau_sac, $keyword) {
        $sql = "SELECT DISTINCT p.* FROM product p  "; 
        
        if (!empty($size) || !empty($mau_sac)) {
